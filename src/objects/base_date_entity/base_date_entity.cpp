...
Base_Date_Entity::Base_Date_Entity(const std::unordered_map<str, std::variant<str, int>> &raw_obj) :
    name(std::get<str>(raw_obj.at("name"))), long_name(std::get<str>(raw_obj.at("longName"))),
    entity_id(std::get<int>(raw_obj.at("id"))), start_date(parse_date(std::get<int>(raw_obj.at("startDate")))),
